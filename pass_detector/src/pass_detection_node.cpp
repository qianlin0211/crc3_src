#include "pass_detection.h"
#include <cstdio>

PassDetection::PassDetection(ros::NodeHandle& node_handle)
...
void PassDetection::detect_image(Mat& cvframe, string modelWeights, string modelConfiguration, string classesFile, std_msgs::Header header)
{
...
void PassDetection::postprocess(Mat& frame, const vector<Mat>& outs)
{
...
vector<String> PassDetection::getOutputsNames(const Net& net)
{
...
{
    ros::init(argc, argv, "pass_detection_node");
    ros::NodeHandle nh("~");

    PassDetection node(nh);
    ros::spin();
    //cv::waitKey(0);
    return 0;
}
